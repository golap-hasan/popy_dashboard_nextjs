...
const generateUserCount = (year: number, monthIdx: number) => 420 + (year - START_YEAR) * 35 + monthIdx * 22;
const generateRevenue = (year: number, monthIdx: number) => 320000 + (year - START_YEAR) * 48000 + monthIdx * 18000;
...
const pendingOrders = orders
    .filter((order) => !["delivered", "cancelled"].includes(order.fulfilmentStatus))
    .sort(
...
const Dashboard = () => {
    ...
    const revenueDataEntry = useMemo(
        () => revenueSeries.find((series) => series.year === earningYear) ?? { data: [], yearlyTotal: 0 },
        [earningYear]
    );
